...
# 把 32 改成了 26，缩小中间的间距
  echo -e " ${C_BCYAN}脚本版本 :${C_RESET} ${C_BMAGENTA}${SBM_VERSION}${C_RESET}\033[22G${C_BCYAN}内核版本 :${C_RESET} ${sb_version}"
  echo -e " ${C_BCYAN}服务状态 :${C_RESET} ${svc_color}${svc_status}${C_RESET}\033[22G${C_BCYAN}入站数量 :${C_RESET} ${inbound_count}"
  echo -e " ${C_BCYAN}UI状态   :${C_RESET} ${ui_color}${ui_status}${C_RESET}\033[22G${C_BCYAN}本地代理 :${C_RESET} ${proxy_color}${proxy_status}${C_RESET}"
...
main_menu() {
  while true; do
    ...
    # 放弃使用原有的 menu_row，直接用带颜色的 echo 排版，彻底解决中文对齐问题
    echo -e " ${C_BCYAN}1.${C_RESET} 安装/升级      ${C_BCYAN}2.${C_RESET} 入站管理"
    echo -e " ${C_BCYAN}3.${C_RESET} 出站管理       ${C_BCYAN}4.${C_RESET} 本地代理(实验)"
    echo -e " ${C_BCYAN}5.${C_RESET} 防火墙管理     ${C_BCYAN}6.${C_RESET} 备份与恢复"
    echo -e " ${C_BCYAN}7.${C_RESET} 服务状态       ${C_BCYAN}8.${C_RESET} 更新脚本"
    echo -e " ${C_BCYAN}9.${C_RESET} 卸载           ${C_BCYAN}0.${C_RESET} 退出"
    echo
